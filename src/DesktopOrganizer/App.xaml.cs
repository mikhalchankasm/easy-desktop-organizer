...
public partial class App : Application
{
    ...
    private static int RestoreAllHidden(Db? db, bool keepMembership)
    {
        ...
        // Намеренно: при dbReadOk=false мы ВСЁ РАВНО восстанавливаем файлы по журналу, а не прерываемся.
        // Журнал — источник восстановления, НЕ зависящий от SQLite (раздел «безопасность данных»):
        // если БД повреждена, именно он должен вернуть файлы на стол. Прерывание до файловых мутаций
        // оставило бы файлы скрытыми как раз тогда, когда независимое восстановление важнее всего,
        // и сломало бы --restore-hidden при битой БД. Цена — флаги БД не очистятся (итог -1, не успех);
        // если БД была недоступна лишь временно, RehideOnStartup на следующем запуске приведёт
        // состояние в согласованность (файл виден → Hide → снова скрыт и записан в журнал).
